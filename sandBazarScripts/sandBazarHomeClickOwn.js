...
(function () {
  ...
  /*** USER CONFIG ***/
  const RETRY_START_TIME_STR = "09:58:20:000"; // Format: HH:MM:SS:ms
  ...
  const TARGET_TEXT = "Abdullapurmet SB Fine";
  ...
  function tryClickOwnTransport() {
    ...
    for (const li of dropdowns) {
      ...
      if (
        toggleAnchor &&
        /SAND\s*BAZAAR\s*BOOKINGS/i.test(toggleAnchor.textContent)
      ) {
        const menu = li.querySelector("ul.dropdown-menu");
        if (menu && menu.children.length) {
          menu.style.display = "block";
          const ownLink = Array.from(menu.querySelectorAll("a")).find(
            (link) =>
